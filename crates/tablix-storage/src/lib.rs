pub trait StorageService {
	fn read(&self, relative_path: impl AsRef<std::path::Path>) -> std::io::Result<Option<String>>;
	fn write(&self, relative_path: impl AsRef<std::path::Path>, content: &str)
	-> std::io::Result<()>;
	fn delete(&self, relative_path: impl AsRef<std::path::Path>) -> std::io::Result<()>;
}
